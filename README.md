...
1. **High Accuracy of Lemmatization:** The tool achieves more than 94% average accuracy in lemmatizing OCS texts by utilizing a comprehensive dictionary of word-lemma pairs. This ensures accurate and reliable lemmatization for a wide range of OCS vocabulary. We achieved this performance on a 5-fold cross-validation, and we are working to enhance its efficiency and robustness on unseen datasets, which can be particularly useful for annotating new datasets.
   
2. **Effective Word-Lemma Mapping:** The tool generates accurate word-lemma mappings in a tabular format, providing clear insights into how words in OCS texts are lemmatized.
   
3. **Efficient Dataset Exploration:** The dataset viewer feature allows users to browse and interact with  Old Church Slavonic texts, which help the user to copy any part of the text.
   
4. **Scalable Solution for Ancient Texts:** The lemmatization tool can handle large volumes of OCS text, making it suitable for researchers and scholars working with ancient linguistic resources across various fields of study.
   
5. **User-Friendly Interface:** With an intuitive GUI, the tool simplifies the process of lemmatization, making it accessible to both experts and non-experts in the field.
